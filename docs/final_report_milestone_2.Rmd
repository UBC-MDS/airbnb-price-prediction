...
```{r predictor-distributions, echo=FALSE, fig.cap="Figure 1: Number of properties by nightly price", out.width = '60%', out.height='60%'}
knitr::include_graphics("../output/number_of_properties_by_price.png")
```
...
```{r neighborhood, echo=FALSE, fig.cap="Figure 2: Number of properties by price and neighborhood", out.width = '60%', out.height='60%'}
knitr::include_graphics("../output/neighborhoods.png")
```
Some neighborhoods do not have any properties listed above a certain price point. For instance, Strathcona and Killarney have no properties listed above \$350/night. Most neighborhoods do not have any properties listed above \$600/night. This lack of training examples for properties of certain prices in certain neighborhoods has implications on our model's ability to predict properties in these "edge cases". Downtown and Kitsilano have some of the highest priced properties, with listings almost consistently up to \$ 1000/night.
...
knitr::include_graphics("../output/price_by_property_type.png")
```
...
baseline <- baseline %>%
  add_column(Models) %>%
  mutate("Computation time (s)" = `Time in seconds`) %>%
  select("Models", "Train MSE", "Validation MSE", "Computation time (s)")

kable(baseline, caption = "Table 1: Baseline performance for four models ")
```
...
kable(optimized, caption = "Table 2: Performance of optimized models")
```
...
```{r property type, echo=FALSE, fig.cap="Figure 4: Residuals by true price for linear regression model", out.width = '60%', out.height='60%'}
knitr::include_graphics("../output/residual_plot.png")
```
